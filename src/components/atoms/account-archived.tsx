...
export function AccountAcrhived({ entry, onClick }: AccountAcrhivedProps) {
    ...
    return (
        <div className="w-full relative text-left px-3 py-3 pl-6 bg-input-bg flex flex-col xs:flex-row items-center justify-between gap-3 sm:gap-5  rounded-2xl">
            ...
            <button
                className="absolute z-0 inset-0 cursor-pointer transition-colors duration-200 ease-in hover:bg-zinc-700/30 rounded-2xl"
                onClick={onClick}
            ></button>
            <button
                onClick={() => {
                ...
                className="z-10 relative cursor-pointer flex items-start gap-1 flex-wrap text-xs sm:text-sm hover:text-white/70 text-text-muted md:justify-end pr-5"
            >
