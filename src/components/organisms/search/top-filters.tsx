...
export default function TopFilters() {
    ...
    if (areFiltersLoading || filters == null || filters.filters == null)
        return <Skeleton />;

    const sortOptions = getSortOptions(filters.filters);
    const currentSortOption = getCurrentSortOption(filters.filters, orderById);
    ...
    return (
        <div className="flex items-center flex-wrap gap-x-3 sm:gap-x-4.5 gap-y-3 sm:gap-y-5 text-transparent animate-pulse mb-3.5">
            {Array.from({ length: 2 }, (_, index) => (
                <div
                    key={`skeleton-top-filters-${index}`}
